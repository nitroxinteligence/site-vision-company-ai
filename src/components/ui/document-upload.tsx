...
export function DocumentUpload({ onChange, maxFiles = 100, maxSize = 25 * 1024 * 1024 }: DocumentUploadProps) {
  ...
  const validateFile = useCallback((file: File): boolean => {
    ...
    if (!validTypes.includes(file.type)) {
      setError(`O arquivo ${file.name} não é um documento válido. Formatos permitidos: PDF, DOC, DOCX, TXT`);
      return false;
    }
  ...
  // Determinar o ícone e cor com base no tipo de arquivo
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "12px" }}>
          ...
          <div>
            <p style={{ fontWeight: "500", marginBottom: "4px" }}>
              Arraste e solte documentos ou <span style={{ color: "#ffffff" }}>clique aqui</span>
            </p>
            <p style={{ fontSize: "0.75rem", color: "rgba(255, 255, 255, 0.6)" }}>
              Suporta PDF, DOC, DOCX, TXT de até {formatFileSize(maxSize)}
            </p>
          </div>
        </div>
      </div>
